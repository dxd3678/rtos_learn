...
- delay 函数在浪费 CPU 的性能。后续需要实现自己的延时函数。
...
- 当任务调用延时函数的时候，让其**<span style="color:#9999FF;">自动让出 CPU</span>**，进行CPU调度。
- 如果当前没有想要运行的任务，自动调度 **“<span style="color:#9999FF;">空闲任务”</span>**。空闲任务有以下几点好处：
  - 降低代码复杂度：在 Pendsv 当中不需要再判断是否不需要进行调度，反正最差也有空闲任务。
  - 添加后台任务，统计任务运行效率等等。
...
- 软件定时器的时间基准：是以 Systick 的定时器周期来的。
  - 如果我们 systick 的周期是 10ms，那么软件定时器的精度最高就是 10ms。

![image-20241118233023175](./Readme.assets/image-20241118233023175.png)

实现要点：

- 在 Systick_Handler 当中轮询每个人任务的计数器是否为0，如果为0，则对其进行调度。
...
- 仿佛每一个 task 都有一个自己的CPU。

![image-20241118235830428](./Readme.assets/image-20241118235830428.png)
